    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="auth-card">
                ...
                <div class="auth-input-wrapper">
                    ...
                    <input type="date" class="auth-input" value="2005-01-01">
                </div>
                <button class="auth-btn">${t('authConfirm')}</button>
            </div>
        `;
        ...
        const validate = () => {
            ...
            if (value === '2005-06-21') {
                ...
                setTimeout(() => {
                    ...
                    if (!successPopup) {
                        ...
                        successPopup.innerHTML = `
                            ...
                            <p>yeyy cantik ku bener, lanjut scroll yang ini ya cantik ku cintaku pasangan hidup ku</p>
                        `;
                    ...
                    setTimeout(() => {
                        const target = window.location.origin + window.location.pathname.replace('index.html', '') + 'birthday-komang-putri.html';
                ...
                const originalDesc = card.querySelector('p').innerHTML;
                card.querySelector('p').style.color = '#ff4d4d';
                card.querySelector('p').innerHTML = t('authError');
                
                setTimeout(() => {
                    card.classList.remove('error-shake');
                    card.querySelector('p').style.color = '#fff';
                    card.querySelector('p').innerHTML = originalDesc;
                }, 2000);
            }
        };
...
function spawnHeartPhotosCentered() {
    console.log("ui_new.js v1.3 active");
    ...
    function spawnNext() {
        if (currentIndex < maxHeartPhotos) {
            ...
            // Animasi foto selesai, munculkan tombol next
            setTimeout(() => {
                const btn = document.createElement('button');
                btn.className = 'next-page-btn';
                btn.innerHTML = (typeof t === 'function' ? t('nextButton') : 'Next ❤️') + ' (v1.3)';
